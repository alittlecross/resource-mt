...
class ForgotPassword {
  static async sendReset (data) {
    let hashed = await bcrypt.hashSync('ketchup', saltRounds)
    let washed = hashed.replace(/\W/g, 'i')
    await databaseForgotPassword.storeResetRequest(data.email, washed)
    let subject = 'Resource Management Tool - Reset Password'
    let html = `<div>A password reset has been requested, please click this link to <a href='${process.env.URL}/${washed}/forgot-password'>reset your password</a>.</div>` +
