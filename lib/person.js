const Database = require('../db/services/person')

class Person {
  static personObject (data) {
    return {
      personId: data.personid,
      firstName: data.firstname,
      surname: data.surname,
      email: data.email,
      roles: [],
      location: data.location,
      status: data.status,
      manager: data.manager,
      skills: []
    }
  }

  static buildPersonObjects (data) {
    let people = []
    data.forEach((row) => {
      if (row.kind === 'details') {
        people.push(this.personObject(row))
      } else if (row.kind === 'role') {
        let result = people.find((x) => x.personId === row.personid)
        result.roles.push(row.role)
      } else {
        let result = people.find((x) => x.personId === row.personid)
        result.skills.push(row.skill)
      }
    })
    return people
  }

  static async add (data) {
    let person = await Database.addPerson(data)

    let string = ''

    if (data.roles !== undefined) {
      if (!Array.isArray(data.roles)) {
        data.roles = [data.roles]
      }
      data.roles.forEach((role) => {
        string += `INSERT INTO personroles VALUES (${person.rows[0].personid}, ${role}); `
      })
    }

    if (data.skills !== undefined) {
      if (!Array.isArray(data.skills)) {
        data.skills = [data.skills]
      }
      data.skills.forEach((skill) => {
        string += `INSERT INTO personskills VALUES (${person.rows[0].personid}, ${skill});`
      })
    }

    let results = await Database.addRolesSkills(string)

    if (Array.isArray(results)) {
      results.unshift(person)
      return results
    } else {
      return [person, results]
    }
  }

  static async getEveryone () {
    let results = await Database.getPeopleRolesSkills()
    return this.buildPersonObjects(results.rows)
  }

  static async alreadyExists (data) {
    let result = await Database.alreadyExists(data.email)

    if (result.rowCount === 0) {
      return {
        status: false
      }
    } else {
      return {
        status: true,
        message: 'email already in use'
      }
    }
  }
}

module.exports = Person
